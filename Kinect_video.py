...
class Kinect_video:
    def __init__(self):
        ...
        self.cameraInfoSub = rospy.Subscriber("/camera/depth_registered/camera_info", CameraInfo,
                                              self.extractCameraInfo)
        self.colorSub = rospy.Subscriber("/rolemanager_to_node", String, self.callbackColor)
        ...
        # Color init
        self.colorlower = np.array([2 - CONST, 50, 50])
        self.colorupper = np.array([2 + CONST, 255, 255])
        # Centroid init
        self.centroid = None
        self.radius = None
    ...
    def callbackColor(self, msg):
        msg = msg.data
        if msg[0] == "0":
            colors = getColor(msg[2:])
            self.colorlower = colors[0]
            self.colorupper = colors[1]
